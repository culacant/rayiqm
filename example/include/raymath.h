...
RMDEF Vector3 QuaternionRotateVector(Vector3 v, Quaternion q)
{
  Vector3 out;
  out.x = v.x*(q.x*q.x+q.w*q.w-q.y*q.y- q.z*q.z) + v.y*(2*q.x*q.y- 2*q.w*q.z) + v.z*(2*q.x*q.z+ 2*q.w*q.y);
  out.y = v.x*(2*q.w*q.z + 2*q.x*q.y) + v.y*(q.w*q.w - q.x*q.x+ q.y*q.y - q.z*q.z)+ v.z*(-2*q.w*q.x+ 2*q.y*q.z);
  out.z = v.x*(-2*q.w*q.y+ 2*q.x*q.z) + v.y*(2*q.w*q.x+ 2*q.y*q.z)+ v.z*(q.w*q.w - q.x*q.x- q.y*q.y+ q.z*q.z);
  return out;
}
#endif  // RAYMATH_H
