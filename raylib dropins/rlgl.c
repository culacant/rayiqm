...
// animated meshes
void rlLoadAnimatedMesh(AnimatedMesh *amesh, bool dynamic)                          // Upload vertex data into GPU and provided VAO/VBO ids
{
    amesh->vaoId = 0;        // Vertex Array Object
    amesh->vboId[0] = 0;     // Vertex positions VBO  << these are the animated vertices in avertices
    amesh->vboId[1] = 0;     // Vertex texcoords VBO
    amesh->vboId[2] = 0;     // Vertex normals VBO    << these are the animated normals in anormals
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float)*3*amesh->vertexCount, amesh->avertices, drawHint);
    glVertexAttribPointer(0, 3, GL_FLOAT, 0, 0, 0);
    glEnableVertexAttribArray(0);
    ...
    // Enable vertex attributes: normals (shader-location = 2)
    if (amesh->anormals != NULL)
    {
        glGenBuffers(1, &amesh->vboId[2]);
        glBindBuffer(GL_ARRAY_BUFFER, amesh->vboId[2]);
        glBufferData(GL_ARRAY_BUFFER, sizeof(float)*3*amesh->vertexCount, amesh->anormals, drawHint);
        glVertexAttribPointer(2, 3, GL_FLOAT, 0, 0, 0);
        glEnableVertexAttribArray(2);
    }
    else
...
RLAPI void rlDrawAnimatedMesh(AnimatedMesh amesh, Material material, Matrix transform)
{
#if defined(GRAPHICS_API_OPENGL_11)
    glEnable(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, material.maps[MAP_DIFFUSE].texture.id);
...
*/
    glVertexPointer(3, GL_FLOAT, 0, amesh.avertices);         // Pointer to vertex coords array
    glTexCoordPointer(2, GL_FLOAT, 0, amesh.texcoords);      // Pointer to texture coords array
    if (amesh.anormals != NULL) glNormalPointer(GL_FLOAT, 0, amesh.anormals);           // Pointer to normals array
/*
    if (mesh.colors != NULL) glColorPointer(4, GL_UNSIGNED_BYTE, 0, mesh.colors);   // Pointer to colors array
*/
    rlPushMatrix();
    ...
    glDisableClientState(GL_VERTEX_ARRAY);                  // Disable vertex array
    glDisableClientState(GL_TEXTURE_COORD_ARRAY);           // Disable texture coords array
    if (amesh.anormals != NULL) glDisableClientState(GL_NORMAL_ARRAY);    // Disable normals array
/*
    if (mesh.colors != NULL) glDisableClientState(GL_NORMAL_ARRAY);     // Disable colors array
*/
    glDisable(GL_TEXTURE_2D);
...
RLAPI void rlUnloadAnimatedMesh(AnimatedMesh *amesh)
{
    if (amesh->vertices != NULL) free(amesh->vertices);
    if (amesh->avertices != NULL) free(amesh->avertices);
    if (amesh->texcoords != NULL) free(amesh->texcoords);
    if (amesh->normals != NULL) free(amesh->normals);
    if (amesh->anormals != NULL) free(amesh->anormals);
//    if (mesh->colors != NULL) free(mesh->colors);
//    if (mesh->tangents != NULL) free(mesh->tangents);
//    if (mesh->texcoords2 != NULL) free(mesh->texcoords2);
...
void rlUpdateAnimatedMesh(AnimatedMesh *amesh)
{
  ...
  glBindBuffer(GL_ARRAY_BUFFER, amesh->vboId[0]);
  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(float)*3*amesh->vertexCount, amesh->avertices);

  glBindBuffer(GL_ARRAY_BUFFER, amesh->vboId[2]);
  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(float)*3*amesh->vertexCount, amesh->anormals);
